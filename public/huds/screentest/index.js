...
function fillPlayers(teams){
    if(teams.left.players){
        for(var i = 0; i < 5; i++){
          fillPlayer(teams.left.players[i],teams.left.players[i].place-1, "left", teams.left.players.length);
          $("#left").find("#player"+(teams.left.players[i].place-1)).css("opacity","1");

        }
    }
    if(teams.right.players){
        for(var i = 0; i < 5; i++){
          fillPlayer(teams.right.players[i],teams.right.players[i].place-1, "right", teams.right.players.length);
          $("#right").find("#player"+(teams.right.players[i].place-1)).css("opacity","1");
        }
    }
}
